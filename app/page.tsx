...
export default function Home() {
  ...
  const handleAddGuest = useCallback(
    async (guestData: Omit<Guest, "id" | "finalGrade">) => {
      try {
        setIsLoading(true);
        setError(null);
  ...
  const handleUpdateGuest = useCallback(
    async (id: string, guestData: Omit<Guest, "id" | "finalGrade">) => {
      try {
        setIsLoading(true);
        setError(null);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      setGuests((prev) =>
        prev.filter((guest) => guest.id !== deleteConfirm.id)
      );
  ...
  return (
    <main className="container">
      ...
      <GuestFormModal
        ...
        onSubmit={
          editingGuest
            ? (data) => handleUpdateGuest(editingGuest.id, data)
            : handleAddGuest
        }
